...
module.exports = (p) => {
  ...
  if (params.background != undefined) {
    if (params.background.length < 1) {
      throw new Error("Background colour Length is less then 1");
    }
  }
  if (params.color === undefined) {
    params.background = DEFAULT_COLOR
  }
...
function fillBackground(ctx, params) {
  ...
  if (params.background) {
    gradient.addColorStop(1, params.background);
  } else{
    for (let i = 0; i < 6; i++) {
      gradient.addColorStop(Math.random() * 0.3 + i * 0.3, randomLightColor(5));
    }
  }
...
function printText(ctx, params) {
  ...
  for (let i = 0; i < params.length; i++) {
    ...
    ctx.fillText(
      value.charAt(i),
      5 + width * i,
      height / 4 + fontSize - 12 + topMargin
    );
  }
}
